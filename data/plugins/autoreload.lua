...
local function check_prompt_reload(doc)
  if doc and doc.deferred_reload then
    core.dialog_view:show("File Changed",
      doc.filename .. " has changed on disk.", {
      { font = style.font, text = "Reload", default_yes = true },
      { font = style.font, text = "Keep Current", default_no = true }
    }, function(item)
      if item.text == "Reload" then reload_doc(doc) end
